...
3. Update This Plan - Edit the target plan's metadata header, appending the link to the `back refs` or `forward refs` row (relative path + short label) without duplicating an existing reference
