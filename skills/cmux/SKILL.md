...
- **Assume the keys are already set up — and never read their values.** By
  ...
  _safely_: `cmux workspace env --workspace <ref> --mask` shows that a var is
...
is an unrelated _named-token rendezvous_ (a manual semaphore you signal yourself);
...
Plain `claude` launches in **ask-for-permission mode**: it will _decline_ to run
