...
function sanitizeTheme(theme: any): Partial<Theme> {
  return {
    ...
    tags: Array.isArray(theme.tags)
      ...
      : [],
    authorId: theme.authorId?.toString(),
    authorName: theme.authorName?.toString(),
  };
}
...
export async function deleteThemeById(
  ...
  try {
    ...
    if (!authorId) {
      return {
        success: false,
        error: "Unauthorized - authorId is required to delete a theme",
      };
    }

    // Only allow deletion by theme author (in a real app, you'd have proper auth)
    if (theme.authorId && theme.authorId !== authorId) {
      return {
        success: false,
