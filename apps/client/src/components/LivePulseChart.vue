<template>
    <div
        class="bg-gradient-to-r from-[var(--theme-bg-primary)] to-[var(--theme-bg-secondary)] px-3 py-4 mobile:py-2 shadow-lg"
    >
        <div class="flex items-center justify-between mb-3 mobile:mb-2">
            <div class="flex items-center gap-3 mobile:gap-2">
                <h3
                    class="text-base mobile:text-xs font-bold text-[var(--theme-primary)] drop-shadow-sm flex items-center"
                >
                    <span class="mr-1.5 mobile:mr-1 text-xl mobile:text-sm"
                        >📊</span
                    >
                ...
                <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    >
                        ...
                        <span
                            class="text-sm mobile:text-xs font-bold text-[var(--theme-primary)]"
                            >{{ uniqueAgentCount }}</span
                        >
                        <span
                            class="text-xs mobile:text-[10px] text-[var(--theme-text-tertiary)] font-medium mobile:hidden"
                            >agents</span
                        >
                    </div>
                    <div
                    ...
                    >
                        <span class="text-lg mobile:text-base">⚡</span>
                        <span
                            class="text-sm mobile:text-xs font-bold text-[var(--theme-text-primary)]"
                            >{{ totalEventCount }}</span
                        >
                        <span
                            class="text-xs mobile:text-[10px] text-[var(--theme-text-tertiary)] font-medium mobile:hidden"
                            >events</span
                    ...
                    >
                        ...
                        <span
                            class="text-sm mobile:text-xs font-bold text-[var(--theme-text-primary)]"
                            >{{ formatGap(eventTimingMetrics.avgGap) }}</span
                        >
                        <span
                            class="text-xs mobile:text-[10px] text-[var(--theme-text-tertiary)] font-medium mobile:hidden"
                            >avg gap</span
            ...
            <div
                class="flex gap-1.5 mobile:gap-1"
                role="tablist"
        ...
        <div ref="chartContainer" class="relative">
            ...
            >
                <p
                    class="text-[var(--theme-text-tertiary)] mobile:text-sm text-base font-semibold"
                >
...
const isEventFiltered = (event: HookEvent): boolean => {
    if (
        props.filters.sourceApp &&
        event.source_app !== props.filters.sourceApp
    ) {
        return false;
    }
    if (
        props.filters.sessionId &&
        event.session_id !== props.filters.sessionId
    ) {
        return false;
    }
    if (
        props.filters.eventType &&
        event.hook_event_type !== props.filters.eventType
    ) {
        return false;
    }
    return true;
...
const processNewEvents = () => {
    ...
    const currentEventIds = new Set(
        currentEvents.map((e) => `${e.id}-${e.timestamp}`),
    );
